...
namespace SBS_BarthLogistic.Controls
{
    ...
    public partial class FMGGvariFilterControl : XtraUserControl, IFMGFindControl
    {
        bool _checked;
        bool alwaysChecked;

        public FMGGvariFilterControl()
        {
            InitializeComponent();
        }
        ...
        public void AddProcParameter(SqlCommand cmd)
        {
            cmd.Parameters.Add("@" + (string.IsNullOrEmpty(ParameterName) ? FieldName : ParameterName),
                SqlDbType.NVarChar).Value = chb.Checked ? txt.Id : "0";
        }
        ...
        void Decolapse()
        {
            myLabel.Visible = txt.Visible = true;
